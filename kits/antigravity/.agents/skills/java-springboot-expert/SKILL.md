...
## Definition of Done
- [ ] Perfil y dependencias alineados al `pom`/`gradle` del repo.
- [ ] Errores de API consistentes (handler existente).
- [ ] Seguridad: no endpoints nuevos anónimos por omisión.

## Errores comunes
- `@Autowired` de campos vs constructor que el repo ya usa.
- `@Transactional` en el controller.
- LazyInitializationException por serializar entidades fuera de sesión.
