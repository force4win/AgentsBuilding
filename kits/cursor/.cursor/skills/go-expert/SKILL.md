...
## Definition of Done
- [ ] Errores envueltos con `%w`; no se ignoran.
- [ ] `context.Context` en I/O y APIs.
- [ ] Layout coherente con el repo (`cmd`/`internal` si ya existe).

## Errores comunes
- Goroutines sin cancelación ni `WaitGroup`.
- Ignorar `err` del `Close`.
- Inventar un framework web distinto al del módulo.
