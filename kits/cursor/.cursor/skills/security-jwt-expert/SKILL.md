...
## Definition of Done
- [ ] Algoritmo de firma explícito (no `none`).
- [ ] Validación de `iss`/`aud`/`exp`.
- [ ] Tokens y claves fuera de logs y del frontend inseguro.

## Errores comunes
- JWT en `localStorage` en una SPA sin BFF ni amenaza modelada.
- HS256 con secreto débil compartido entre servicios.
- Confiar en `roles` del token sin chequear el recurso.
